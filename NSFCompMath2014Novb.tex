...
\usepackage{stackrel,natbib,tabularx,enumitem,xspace}
...
\newcommand{\refprobac}{\hyperref[DesignerKernelssubsec]{Research Project~1.3}}
\newcommand{\refprobb}{\hyperref[SectGAIL]{Research Project~2}}
\newcommand{\refprobba}{\hyperref[Conessubsec]{Research Project~2.1}}
\newcommand{\refprobbb}{\hyperref[Integrationsubsec]{Research Project~2.2}}
\newcommand{\refprobc}{\hyperref[SectGuaranteedKernels]{Research Project~3}}
\newcommand{\refprobca}{\hyperref[DatadrivenErrorsubsec]{Research Project~3.1}}
\newcommand{\refprobcb}{\hyperref[DatadrivenParametersubsec]{Research Project~3.2}}
\newcommand{\refprobcc}{\hyperref[DatadrivenDesignsubsec]{Research Project~3.3}}
\newcommand{\refprobd}{\hyperref[SectApplications]{Research Project~4}}
\newcommand{\refprobda}{\hyperref[ENOsubsec]{Research Project~4.1}}
\newcommand{\refprobdb}{\hyperref[MEEGsubsec]{Research Project~4.2}}
\newcommand{\refprobdc}{\hyperref[TruncHSsubsec]{Research Project~4.3}}
...
\end{equation}
\end{subequations}
For practical applications the matrices $\mLambda_2$ and $\mPhi_2$ will therefore have to be truncated appropriately (see Sect.~\ref{TruncHSsubsec}). We refer to the Hilbert--Schmidt SVD \eqref{HSSVD} as a ``formal decomposition'' since it is important to note that the matrices $\mPsi$, $\mLambda_1$, $\mLambda_2$, $\mPhi_1$ and $\mPhi_2$ are not found by manipulating $\mK$, but are formed directly from the eigenvalues and eigenfunctions of $\cK$. In fact, the ill-conditioned matrix $\mK$ never needs to be formed, nor is precise knowledge of the closed form of the kernel $K$ essential. As long as the eigenfunctions and eigenvalues are known, the Hilbert--Schmidt SVD enables us to formulate stable and efficient algorithms for the associated kernel approximation problems \citep{CavorettoEtAl14}.
...
\subsection*{Research Project 1.3. Designer Kernels}\label{DesignerKernelssubsec}
...
\subsection*{Why Cones}\label{Conessubsec}
\subsection*{Adaptive Algorithms for Integration Problems ($\INT$)}\label{Integrationsubsec}  
...
\section*{Research Project 3. Combining Kernel Methods with Guaranteed and Automatic Algorithms}\label{SectGuaranteedKernels}

\subsection*{Research Project 3.1. Data-Driven Error Estimation}\label{DatadrivenErrorsubsec} 
One may estimate the error of the meshfree approximation \eqref{rbfapprox} in a data-driven way by using cross-validation (CV).  The parameters of the kernel may be estimated by maximum likelihood estimation (MLE).  However, neither of these methods have finite-sample size guarantees of success.
...
\subsection*{Research Project 3.2. Data-Driven Parameter Estimation}\label{DatadrivenParametersubsec}
...
\subsection*{Research Project 3.3. Data-Driven Designs}\label{DatadrivenDesignsubsec}
...
\section*{Research Project 4. Applications of Kernels}\label{SectApplications}
\subsection*{Research Project 4.1. Numerical Solution of PDEs via Kernel ENO Methods}\label{ENOsubsec}
...
\subsection*{Research Project 4.2. Using Kernels to Solve the MEG and EEG Forward Problems}\label{MEEGsubsec}
...
\subsection*{Research Project 4.3. Low-rank Approximations for Kernel Classification}\label{TruncHSsubsec}
