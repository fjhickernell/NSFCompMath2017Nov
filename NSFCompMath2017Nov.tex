...
\subsubsection{Globally  Adaptive Quasi-Monte Carlo (QMC) Cubature} \label{QMCsec}
The PI, LlAJR, and DL 
have 
...
globally adaptive QMC algorithms described in Sect.\ \ref{QMCsec} to situations with arbitrarily large 
...
evaluating $f_{d_L}(x_{d_L})$ is often proportional to $d_L$.  If $A_{n,d}$ represents an IID MC, 
...
Two alternative methods for approximating $S(f)$ are the multi-level method (MLM) \cite{Gil14a} 
and the 
multivariate 
decomposition method (MDM) \cite{Was13b}.  The function $f$, which depends on $\bx=(x_1, x_2, 
\ldots) \in 
\Omega^\naturals$, 
...
for 
approximating $S(f)$ are \cite{Gil14a, Was13b}
\begin{gather*}
...
The challenge in constructing a theoretically sound adaptive algorithm is determining good choices 
...
sequence node QMC sampling.  In particular, we expect that smoother kernels and higher order nets 
...
The Bayesian cubature framework may also be used for function approximation.  This approach is 
known as kriging in the statistics literature or kernel/radial basis function approximation in the 
numerical analysis literature.  Again the We will develop adaptive function approximation 
doit Roshan
...
has expertise in (quasi-)Monte Carlo methods, kernel-based methods, information-based complexity 
...
applied mathematicians through the SAMSI QMC program.
