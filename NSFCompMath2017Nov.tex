...
bound, 
and consequently the computational cost.  For \IIDMC cubature, a nearly optimal $\bbeta$ is 
...
this section.  Female students mentored include YD, LJ, XT, and XZ.   There have also been a 
number of 
...
\cite[Sect.\ 5.4]{BraPet11a}, we will develop a data-driven upper bound on the roughness of the 
function, 
...
\eqref{errorcrit} or \eqref{generrorcrit}.
...
quadratic approximations.  We emphasize that while being locally adaptive, our algorithm finds the 
global optimum.
...
\]
where $f_d$ is an  approximation to $f$ that depends only on the first $d$ variables.  Such 
problems arise as the expected value of a functional of the solution of a 
stochastic differential equation (SDE), where $d$ is the number of time steps in the numerical 
approximation of the SDE.  

One method for approximating $S(f)$ is to approximate the finite 
...
did not allow the desired answer to be a function of several integrals.  We will generalize LJ's work to 
...
sample size needed is determined sequentially by increasing it and then estimating the error of the 
...
We will employ the matched \QMC node sequences with the well-matched kernels that we 
have found successful for cubature to the problem of multivariate function approximation.  We will 
...
huge body of recent work on the the \emph{tractability} of integration and function approximation 
problems, essentially, under what conditions the computational cost of satisfying \eqref{errorcrit} is 
$\Order(d^q \varepsilon^{-p})$ as $d \to \infty$.  The PI has contributed to this literature 
...
\dif \bx
\right\}, 
\frac{\partial^{\abs{\fu}}f}{\partial \bx_u}(\bx_u,\bzero), \\
\text{e.g., } f(x_1,x_2) = (x_1^2+1)\cos(x_2), \ \ \partial^{\emptyset}f =  1, \ \ \partial^{\{1\}} f = 2 x_1, \ \ 
\partial^{\{2\}}f  = \sin(x_2), \ \  \partial^{\{1,2\}}f  = 2x_1\sin(x_2),
\end{gather*}
...
$f_u(\bx)$ is on $f(\bx)$.  The literature cited above provides necessary and sufficient conditions 
...
In some contexts one has a sense of how to order the coordinates in roughly order of decreasing 
importance, but in the contexts we have in mind, such an ordering does not exist.  The helpful ideas 
from the tractability literature can be applied only after an initial probe of the function indicates 
which coordinates are more important and which are less so.

Two of the \SAMSIQMC working groups that the PI is involved in are exploring this problem of 
placing an initial set 
of nodes to evenly probe the domain of $f$, followed by node placement that favors the important 
coordinate directions.  In essence, this will help us determine the coordinate weights and then allow 
us to use the ideas from tractability to solve the integration or optimization problem.  

Our new algorithms will 
...
to 
support two summer REU students per year.  Having run our small REU program for several  years 
has given us some visibility and prompts inquiries from prospective participants well before we 
even announce our latest program 
...
\emph{Preparing Students for Academic Careers.} We consider mentoring to be a multi-faceted and 
potentially long-term process continuing even after the mentee has moved on from IIT.  We 
continue to have contact with many of our former students, and in particular we continue to have 
collaborations with YD, Qi Ye, and Yiou Li, who are pursuing academic careers.  

\emph{Preparing Students for Industry Careers.}
In addition to preparing students for the academic landscape, we also help current students land 
competitive jobs in the business world. Our training in the areas of algorithm development and 
coding tends to give our students the needed edge. For example, WM graduated from Brown U in 
2013 and then joined an internet startup company. LlAJR and XZ are working in the financial 
services industry and  LJ is working in marketing analytics.  All of them are developing and testing 
quantitatively sophisticated and computationally intensive models.
...
The research supported by this grant will result in publications in peer-reviewed journals in a broad 
spectrum of applied mathematics, computer science, statistics and science or engineering. These 
journals will include both those that emphasize theory and those that emphasize applications.

\emph{Promoting Cones} The idea of guaranteed, adaptive algorithms via cones of input functions 
...
complexity theorists who analyze the lower bounds on the complexity of numerical problems.  

\emph{Bridging Mathematics and Statistics}
This project touches on topics that are of interest to the statistics community: kriging, Monte Carlo 
methods, and design of experiments.  Historically, there has been relatively little interaction between 
numerical analysts and statisticians.  We have and will continue to engage the statistics community 
...
new results on guaranteed multi-level and (quasi-)Monte Carlo sampling (see ).
...
\emph{Reaching Out.}
GEF and FJH both have a record of reaching out to high school students which we plan to continue. 
The website 
\href{http://math.iit.edu/~openscholar/meshfree/}{\nolinkurl{math.iit.edu/~openscholar/meshfree/}} 
helps manage our internal and external communication and dissemination of research findings. 
Advertising for the summer REU experiences is also facilitated via this website.  Information about 
GAIL is at \href{http://code.google.com/p/gail}{\nolinkurl{code.google.com/p/gail}}.
