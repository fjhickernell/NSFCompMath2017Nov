...
\emph{Adaptive Bayesian Cubature.}  Recent research by the PI and JR has assumed 
...
Two alternative methods for approximating $S(f)$ are the multi-level method (MLM) and the 
multivariate 
decomposition method (MDM).  The function $f$, which depends on $\bx=(x_1, x_2, \ldots) \in 
\Omega^\naturals$, 
...
\end{equation*}
HereThis decomposition is not unique and may be based on convenience.  Each $f_{\fu}$ depends 
only 
on the coordinates with indices in $\fu$.  If $A_{n,d}$ represents an IID MC, QMC or other cubature 
using $n$ nodes on the $d$-dimensional domain, then the MLM and MDM cubatures for 
approximating $S(f)$ is 
\begin{gather*}
...
n_\fu\right)\\
\end{gather*}
At first glance, these costs may look greater than the $\Order(d_L n_L)$ cost of the cubature 
$A_{n_L,d_L}(f)$ for approximating $S(f)$.  However, the MLM and MDM are actually more 
economical for the commonly occurring integrals where the low dimensional cubatures require 
larger numbers of nodes, 
but the higher dimensional cubatures actually require fewer numbers of nodes.
...
These must be chosen so that the error due to ignoring some pieces of the integrand is negligible 
and so that the total cost of the cubatures is efficient. 
Once  $\bd$ or$\fU$ are set, our adaptive cubatures in Sect.\ \ref{QMCsec} can be used to 
...
discover the unknown, something that is not taught well in a classroom. We request funds to 
support two summer REU students per year.  Having run our small REU program for several  years 
has given us some visibility and is prompting inquiries from prospective participants well before we 
...
potentially long-term process continuing even after the mentee has moved on from IIT.  For 
example, MM was an undergraduate student at IIT who collaborated with GEF during his PhD 
studies at Cornell U.  We will continue to mentor him as senior person for this proposal.
...
\emph{Giving Short Courses and Invited Lectures}
...
has broad potential application.  We will be promoting this idea among numerical analysts who 
...
\emph{Promoting the Well-being of the Broader Society}
...
\emph{Writing Textbooks and Survey Papers}
